...
UniValue sendtomainchain_pak(const JSONRPCRequest& request)
{
    ...
    LogPrintf("[iwan][wsign] secp256k1_whitelist_signature_serialize() secp256k1_ctx = %s\n", HexStr((unsigned char*)secp256k1_ctx, ((unsigned char*)secp256k1_ctx) + secp256k1_context_preallocated_clone_size(secp256k1_context)));
    // LogPrintf("[iwan][wsign] secp256k1_whitelist_signature_serialize() paklist.OnlineKeys() = %s\n", HexStr((unsigned char*)&paklist.OnlineKeys()[0], ((unsigned char*)&paklist.OnlineKeys()[0]) + 64 * 29));
    // LogPrintf("[iwan][wsign] secp256k1_whitelist_signature_serialize() paklist.OfflineKeys() = %s\n", HexStr((unsigned char*)&paklist.OfflineKeys()[0], ((unsigned char*)&paklist.OfflineKeys()[0]) + 64 * 29));
    ...
    NullData nulldata;
    nulldata << std::vector<unsigned char>(genesisBlockHash.begin(), genesisBlockHash.end());
    LogPrintf("[iwan][send] genesisBlockHash = %s\n", HexStr(genesisBlockHash.begin(), genesisBlockHash.end()));
    nulldata << std::vector<unsigned char>(mainchain_script.begin(), mainchain_script.end());
    LogPrintf("[iwan][send] mainchainScript = %s\n", HexStr(mainchain_script.begin(), mainchain_script.end()));
